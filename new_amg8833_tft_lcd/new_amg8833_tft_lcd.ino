...
void loop() {
  ...
  uint16_t boxsize = min(tft.height() / INTERPOLATED_COLS, tft.width() / INTERPOLATED_COLS);
  //uint16_t boxsize = 20;
  ...
  tft.setTextSize(3);
  tft.fillRect(70, 300, 120, 30, BLACK );
  tft.print("MAX:");
  tft.setTextColor(WHITE);
  tft.fillRect(100, 300, 120, 30, BLACK );
  tft.print(pix_max);
  tft.print(" C");
  ...
  if (k >= akurasi && pix_max > set) {
    //tft.setCursor(0, 420);
    tft.fillRect(410, 260, 50, 50, GREEN );
    digitalWrite(motor, 1);
    digitalWrite(valve, 0);
  }
  else {
    tft.fillRect(410, 260, 50, 50, RED );
...
void drawpixels(float *p, uint8_t rows, uint8_t cols, uint8_t boxWidth, uint8_t boxHeight, boolean showVal) {
  ...
  for (int y = 0; y < rows; y++) {
    for (int x = 0; x < cols; x++) {
      ...
      uint16_t color;
      color = val * 2;
      tft.fillRect(boxWidth * x + 90, boxHeight * y + 30, boxWidth, boxHeight, camColors[colorIndex]);
...
void colorbar() {
  ...
  for (int i = v; i < 320 + v; i++) {
    u = i - v;
    tft.fillRect(i, 0, 1, 25, camColors[(int)(u * 0.8)]);
  }
  tft.setCursor(30, 5);
  tft.setTextSize(2);
  tft.print(MINTEMP);
